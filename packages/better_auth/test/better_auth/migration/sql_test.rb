...
class BetterAuthMigrationSQLTest < Minitest::Test
  ...
  def test_renders_sql_migration_with_plugin_schema
    ...
    sql = BetterAuth::Migration::SQL.render(config, dialect: :postgres, source: "better_auth-test")
  ...
  def test_executes_pending_sql_migrations_once
    Dir.mktmpdir("better-auth-core-migrate") do |dir|
      ...
      BetterAuth::Migration::SQL.migrate(auth, migrations_path: migrations_path, package_name: "better_auth-test")
      BetterAuth::Migration::SQL.migrate(auth, migrations_path: migrations_path, package_name: "better_auth-test")
  ...
  def test_does_not_split_semicolons_inside_dollar_quoted_blocks
    ...
    assert_equal [
      "DO $$\nBEGIN\n  RAISE NOTICE 'a;b';\nEND\n$$",
      "CREATE TABLE audit_log (id text PRIMARY KEY)"
    ], BetterAuth::Migration::SQL.statements(sql)
  end

  def test_normalizes_common_database_dialect_aliases
    assert_equal :postgres, BetterAuth::Migration::SQL.normalize_dialect("postgresql")
    assert_equal :sqlite, BetterAuth::Migration::SQL.normalize_dialect("sqlite3")
  end
end
